...
export default function EditConnectionScreen() {
  ...
  const { state, updateConnection } = useAppState();
  const connection = state.connections.find((item) => item.id === connectionId);

  const [name, setName] = useState(connection?.name ?? '');
  const [url, setUrl] = useState(connection?.websocketUrl ?? '');
  ...
  const onSave = () => {
    ...
    const ok = updateConnection(connection.id, name.trim() || fallbackName, normalizedUrl);
    if (!ok) {
      Alert.alert('Update failed', 'Connection no longer exists.');
      router.replace('/');
      return;
    }
    router.replace(`/connection/${connection.id}` as never);
  };
